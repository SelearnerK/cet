  ...
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange"
      ...
      <el-table-column width="70">
        <template slot-scope="scope">
          <el-radio :label="scope.row.id" v-model="radioId" @change="handleCurrentChange(scope.row)"></el-radio>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('action.operation')" v-if="showOperation" header-align="center" align="center" width="300">
        <template slot-scope="scope">
          <kt-button icon="el-icon-s-promotion" label="发送邮件" :perms="permsNotify" :size="size" v-if="showNotify" type="success"
            @click="handleNotify(scope.$index, scope.row)" />
          <kt-button icon="fa fa-edit" :label="$t('action.edit')" :perms="permsEdit" :size="size"
            @click="handleEdit(scope.$index, scope.row)" />
          <kt-button icon="el-icon-document-copy" label="另存" :perms="permsSaveAs" :size="size" v-if="showSaveAs"
            @click="handleSaveAs(scope.$index, scope.row)" />
    ...
    <!-- <div class="toolbar" style="padding:10px;">
      ...
      <el-pagination layout="total, prev, pager, next, jumper" @current-change="refreshPageRequest" 
        :current-page="pageRequest.pageNumber" :page-size="pageRequest.pageSize" :total="data.totalSize" style="float:right;">
      </el-pagination>
    </div> -->
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleNotify: function (index, row) {
      ...
      // 编辑
      handleSaveAs: function (index, row) {
        this.$emit('handleSaveAs', {
          index: index,
          row: row
        })
      },
